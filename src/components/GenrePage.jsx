...
const GenrePage = ({ blogs }) => {
    const { genre } = useParams();
    const filteredBlogs = blogs.filter(blog => blog.genre === genre);

    return (
        <div className="flex-1 p-6">
            <h2 className="text-2xl font-bold">{genre} Blogs</h2>
            {filteredBlogs.length > 0 ? (
                <div className="grid grid-cols-3 gap-4 mt-6">
                    {filteredBlogs.map((blog, index) => (
                        <Link to={`/blog/${index}`} state={{ blog }} key={index}>
                            <div className='relative bg-gray-800 rounded-lg overflow-hidden h-48 flex items-center justify-center'>
                                <img src={blog.image || 'https://via.placeholder.com/300'} className='absolute inset-0 w-full h-full object-cover' alt={blog.title} />
